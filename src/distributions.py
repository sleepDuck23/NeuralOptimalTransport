...
class Mix3GaussiansSampler(Sampler):
    def __init__(self, std=1, r=12, dim=3, device='cuda'):
...
class HeavyTailedSampler(Sampler):
    def __init__(self, dim=3, df=2, device='cuda'):
        super().__init__(device=device)
        assert dim == 3
        self.dim = dim
    ...
    def sample(self, batch_size=10):
        with torch.no_grad():
            batch = torch.tensor(np.random.standard_t(self.df, size=(batch_size, self.dim)), dtype=torch.float32, device=self.device)
        return batch
...
class SwissRoll3DSampler(Sampler):
    ...
    def sample(self, batch_size=10):
        batch, _ = datasets.make_swiss_roll(n_samples=batch_size, noise=0.8)
        batch = batch.astype('float32') / 7.5
        return torch.tensor(batch, device=self.device)
